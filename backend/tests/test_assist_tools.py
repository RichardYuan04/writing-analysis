...
# ── /assist/continue：续写（走 DeepSeek，注入 SOUL，把 hint 带进 user）──

def test_continue_returns_result_and_uses_deepseek(client, mock_deepseek):
    mock_deepseek.set_text("夜更深了，便利店的灯还亮着，像替谁守着一盏。")
    ...
    cap = mock_deepseek.captured
    assert cap["max_tokens"] >= 2000          # 推理模型留足预算（推理 + 续写）


def test_continue_injects_soul_taboo_and_hint(client, mock_deepseek, db):
    mock_deepseek.set_text("续写内容")
    ...
    assert "朝孤独的反面写" in cap["user"]
    assert "原文一句" in cap["user"]


def test_continue_rejects_empty_text(client, mock_deepseek):
